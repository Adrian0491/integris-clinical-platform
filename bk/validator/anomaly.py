...
#from cdct.schemas import FINDINGS_DTYPES, concat_findings, empty_findings

NUMERIC_COLS  = ["age", "systolic_bp", "treatment_dose"]
REQUIRED_COLS = ["age", "systolic_bp", "treatment_dose", "visit_date"]

# (inclusive_min, inclusive_max) — use None for no bound
RULES: dict[str, tuple[float | None, float | None]] = {
    "age":            (18.0,  100.0),
    "systolic_bp":    (90.0,  180.0),
    "treatment_dose": (0.001, None),   # must be > 0
}
...
def apply_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "visit_date" in df.columns:
        df["date_valid"] = df["visit_date"].notna().astype(int)
...
def to_findings(df: pd.DataFrame) -> pd.DataFrame:
    ...
    from cdct.schemas import FINDINGS_COLUMNS
